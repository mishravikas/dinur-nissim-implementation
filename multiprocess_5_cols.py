...
sigma = 2 
# e_dynamic = 2*sigma
sqlite_file = 'db_6_cols.sqlite'    # name of the sqlite database file
...
# 	sql = "SELECT sum(c1) FROM `binary_data` WHERE (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d and `c6`=%d)" %(queries[m][0],queries[m][1],queries[m][2],queries[m][3],queries[m][4])
# 	len_noisy = execute_query(sql,cursor)
# 	var_lens.append(len_noisy[0])

# print "==========="
# print var_lens

total_count = execute_query("Select sum(c1) from `binary_data`",cursor)[0]
	...
	while not successful:
		...
		def A(q):
			...
			for idx, val in enumerate(q):
				if(val):
					if(first_clause):
						sql += "(`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d and `c6`=%d)" %(queries[idx][0],queries[idx][1],queries[idx][2],queries[idx][3],queries[idx][4])
						first_clause = 0
					else:
						sql += " or (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d and `c6`=%d)" %(queries[idx][0],queries[idx][1],queries[idx][2],queries[idx][3],queries[idx][4])
...
try:
	...
	l_d_values = pool.map(_worker, range(n_runs))
finally:
	pool.close()
...
print "Calculated Distribution: ", latest
true_distribution = []
for row in queries:
	# print row
	sql = "SELECT sum(c1) FROM `binary_data` WHERE (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d and `c6`=%d)" %(row[0],row[1],row[2],row[3],row[4])
	cursor.execute(sql)
	result = cursor.fetchone()
	true_count = result[0]
	true_distribution.append(true_count)

print "True Distribution:      ", true_distribution

print "Frequencies: ", list_fs
