...
sigma = 1.5 
# e_dynamic = 2*sigma
sqlite_file = 'db_800_5_cols.sqlite'    # name of the sqlite database file
...
for m in range(0,len(queries)):
	sql = "SELECT sum(c1) FROM `binary_data` WHERE (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d)" %(queries[m][0],queries[m][1],queries[m][2],queries[m][3])
	...
	while not succesfull:
		...
		def A(q):
			...
			for idx, val in enumerate(q):
				if(val):
					if(first_clause):
						sql += "(`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d)" %(queries[idx][0],queries[idx][1],queries[idx][2],queries[idx][3])
						first_clause = 0
					else:
						sql += " or (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d)" %(queries[idx][0],queries[idx][1],queries[idx][2],queries[idx][3])
...
for row in queries:
	# print row
	sql = "SELECT sum(c1) FROM `binary_data` WHERE (`c2`=%d and `c3`=%d and `c4`=%d and `c5`=%d)" %(row[0],row[1],row[2],row[3])
	cursor.execute(sql)
	result = cursor.fetchone()
	true_count = result[0]
